...
* 같은 목적을 달성하는 데 여러 가지 방법이 존재함.
* 예. 위치 정보는 구하는 3가지 방법(GPS, 휴대전화 삼각측량, 와이파이)
* 동일한 인터페이스를 구현한 전략을 상황에 따라 선택.
* 전략의 선택(사용?) 방법
  * 단일 전략
  * (분석을 통한 적합한) 단일 전략
  * **점진적인 향상 선택**
    * 빠른(부정확한) 전략 결과로 우선 업데이트 후 느린(정확한) 전략 결과로 업데이트
  * 무작위 선택
* 전략 패턴의 전통적인 구현에서는 각 전략에 대한 메소드 **시그니처**가 동일함
* (자바스크립트에서 매우 유용) 인터페이스가 필요 없기 때문에 훨씬 **간단하게 구현**할 수 있음. **동일한 모양의 객체를 반환할 필요 없음**.
...
* 전략패턴은 알고리즘 전체를 대체하고, 템플릿 메소드 패턴은 알고리즘 일부는 공유, 일부는 대체함(override 확장)
* 알고리즘의 점진적 향상의 원칙을 적용할 수 있게 함.(상속 트리 구조 구축)
* 상속을 사용하기 때문에 부모 클래스와 강하게 결합되어 있음.
* (Javascript에서는 추상 메소드 개념이 없음. 책에서는 Throw 활용.)
...
* 해석자
  * eval 위험성
* 메멘토
  * 상태 변화, 누가 변화 주었는지 추적 가능.
  * 객체 변경을 정확하게 확인하고 복구 가능.
* 감시자
  * 청취자가 정확하게 등록되지 않은 경우 **메모리 누수** 발생 가능
